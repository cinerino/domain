...
export function download(params: {
    ...
    return async (repos: { transaction: TransactionRepo }): Promise<json2csv.Transform<any>> => {
        ...
        switch (params.format) {
            case factory.encodingFormat.Text.csv:
                return new Promise<json2csv.Transform<any>>((resolve) => {
                    ...
                    const processor = inputStream.pipe(transform);
                    resolve(processor);
                });

            default:
                throw new factory.errors.NotImplemented('Unimplemented format');
        }
    };
}
// tslint:disable-next-line:no-single-line-block-comment
